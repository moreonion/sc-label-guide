...
import {_COLUMNS_, _ROUTE_, _ORDERBY_, _OPERATORS_} from '../config/config.js'
...
const isAllowedOperator = (col, op) => {
  const ops = _COLUMNS_.columnMeta[columnValueMap(col)].ops
  const mapOp = _OPERATORS_.opEncMapRev[op]
  if(ops !== undefined) {
    return ops.find(o => o === mapOp) !== undefined
  } else {
    return _OPERATORS_.ops.find(o => o === mapOp) !== undefined
  }
}
...
const queryValidator = (queryArr, _op) => {
  ...
  // Operator must be allowed on column AND
  // Value must exist
