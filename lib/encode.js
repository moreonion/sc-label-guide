 ...
 */
export function encodeQuery(queryObj, columnMap=id, opMap=id, exprDelim=',', splitDelim='-', listDelim=',') {
  const encodeList = (listVal, column) => listVal.reduce((accum, v) => {
    if(accum.length === 0) {
      return `${column}${splitDelim}${v}`
    } else {
      return `${accum}${listDelim}${column}${splitDelim}${v}`
    }
  }, '')
  ...
  return queryObj['$and'].reduce((accum, subQuery) => {
    ...
    if(isListOperator(op)) {
      if(accum[mapOp]) {
        accum[mapOp] = `${accum[mapOp]}${listDelim}${encodeList(val, columnMap(column))}`
      } else {
