...
export function decodeQueryFactory(opMap=id, columnMap=id, parseFunc=(_, snd) => snd, exprDelim=',', splitDelim='-') {
  return function decodeQuery(encQuery, op) {
    ...
    if(isListOperator(mapOp)) {
      const colMap = exprs.map(expr => decodeArray(expr, splitDelim)).reduce((accum, [column, val]) => {
        const colMapped = columnMap(column)
        if(accum[colMapped]) {
          accum[colMapped].push(val)
        } else {
          accum[colMapped] = [val]
        }
        return accum
      ...
      return reduce(colMap, (accum, right, left) => {
        accum['$and'].push({[left]: {[mapOp]: right}})
        return accum
      }, {$and: []})
    } else {
