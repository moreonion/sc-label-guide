 ...
 */
export function deserializeOrderBy(serOrderBy, serOrderDir, dirToken='asc', splitDelim=',') {
  const cols = this.deserializeArray(serOrderBy, splitDelim)
  const dirs = this.deserializeArray(serOrderDir, splitDelim)
...
export function deserializeFilterFactory(opMap=id, fieldMap=id, parseFunc=id, exprDelim=',', splitDelim='-') {
  return function deserializeFilter(serFilter, op) {
    const exprs = this.deserializeArray(serFilter, exprDelim)
