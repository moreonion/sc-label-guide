...
export function defaultApiEncode(select, query, search, orderBy, limit, page) {
  // const addSelect = 'meets_criteria'
  // const addInclude = 'meets_criteria.criterion.all'
  const addSelect = ''
  const addInclude = ''

  const [encSelect, encInclude] = encodeApiSelect(select, _API_.listDelim)

  const qSelect = encSelect.length === 0 ? addSelect : `${encSelect}${_API_.listDelim}${addSelect}`
  const qInclude = encInclude.length === 0 ? addInclude : `${encInclude}${_API_.listDelim}${addInclude}`
